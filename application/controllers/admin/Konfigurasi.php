...
class Konfigurasi extends CI_Controller{
    ...
    public function index()
    {
        ...
        $result = $this->konfig_model->listing($search,$offset,$total);

        $jml = $this->db->get('konfigurasi');
        $data['jmldata'] = $jml->num_rows();

        $config['uri_segment'] = 4;
        $config['base_url'] = base_url().'admin/konfigurasi/index/';
    ...
    public function tambah(){
        ...
        if ($this->form_validation->run()) {
            ...
            if( $this->upload->do_upload('gambar')) {
                ...
                $data = array(  'nama_perusahaan'   =>  $i->post('nama_perusahaan'),
                                ...
                                'fax'               =>  $i->post('fax'),
                                'jam_kantor'        =>  $i->post('jam_kantor'),
                            );
                // var_dump($data); die();
                $this->konfig_model->tambah($data);
                $this->session->set_flashdata('sukses','konfigurasi berhasil ditambahkan');
        ...
        $this->load->view('admin/header',$data);
        $this->load->view('admin/konfigurasi/tambah',$data);
        $this->load->view('admin/footer');
    }

    public function edit($id)
    {
        $konfigurasi = $this->konfig_model->detail($id);
        // validasi
        $valid = $this->validation_rule();
        $this->form_validation->set_rules($valid);
            ...
            if (!empty($_FILES['gambar']['name'])) {
                ...
                if($this->upload->do_upload('gambar')) {
                    ...
                    if ($konfigurasi->gambar != "") {
                        ...
                        unlink('assets/upload/image/thumbs/'.$konfigurasi->gambar);
                    }
                    // end hapus
                    ...
                    $this->konfig_model->edit($id,$data);
                    $this->session->set_flashdata('sukses','konfigurasi berhasil di edit');
                    redirect(base_url('admin/konfigurasi'));
                } // End masuk database
            } else {
        ...
        $this->load->view('admin/header',$data);
        $this->load->view('admin/konfigurasi/edit', $data);
        $this->load->view('admin/footer');
    }

    public function hapus($id)
    {
        $data = $this->konfig_model->detail($id);
        // var_dump($data->gambar); die();
        if ($data->gambar != "") {
            unlink('assets/upload/image/'.$data->gambar);
            unlink('assets/upload/image/thumbs/'.$data->gambar);
        }
        $this->konfig_model->delete($id);
        $this->session->set_flashdata('sukses','Data Kategori konfigurasi telah dihapus');
        redirect(base_url('admin/konfigurasi'));
    }
